...
bool ReservationTable::isChannelAvailable(const int port_in, const int port_out)
{
    ...
    // check if port_in is reserving other port_out
    for (map<int,int>::iterator i=rtable.begin(); i!=rtable.end(); i++)
    {
        if (i->second == port_in)
            return false;		
    ...
    // port_in is idle, check port_out
    return (rtable[port_out]==NOT_RESERVED);
}
...
int ReservationTable::getInputOccupyingOutput(const int port_out)
{
    return rtable[port_out];
}
